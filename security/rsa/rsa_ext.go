...
// 公钥解密byte
func pubKeyByte(pub *rsa.PublicKey, in []byte) ([]byte, error) {
	k := (pub.N.BitLen() + 7) / 8
	if len(in) <= k {
		...
		if err := pubKeyIO(pub, bytes.NewReader(in), out); err != nil {
			return nil, err
		}
...
// 私钥加密byte
func priKeyByte(pri *rsa.PrivateKey, in []byte) ([]byte, error) {
	k := (pri.N.BitLen() + 7) / 8
	k = k - 11
	if len(in) <= k {
		...
		iv := make([]byte, k)
		out := bytes.NewBuffer(iv)
		if err := priKeyIO(pri, bytes.NewReader(in), out); err != nil {
			return nil, err
		}
		return ioutil.ReadAll(out)
	}
