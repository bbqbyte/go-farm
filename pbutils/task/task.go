...
import (
	...
	rbqueue "keywea.com/cloud/pblib/pbutils/rbqueue"
)
...
type Runner struct {
	...
	state      state
	namedQueue map[string]*rbqueue.Queue
}
...
func NewRunner() *Runner {
	t := &Runner{
		...
		state:      running,
		namedQueue: make(map[string]*rbqueue.Queue),
...
func (s *Runner) startWorker(name string, q *rbqueue.Queue) (uint64, error) {
...
func (s *Runner) getDefaultQueue() *rbqueue.Queue {
	return s.getNamedQueue(defaultQueueName)
}

func (s *Runner) getNamedQueue(name string) *rbqueue.Queue {
